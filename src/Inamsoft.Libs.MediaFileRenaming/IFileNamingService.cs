...
public interface IFileNamingService
{
    ...
    /// <summary>
    /// Renames a file based on the provided rename request and settings
    /// by using the metadata information the file might have.
    /// </summary>
    /// <param name="renameFileRequest">The information about the source file.</param>
    /// <param name="settings">The settings that controls the behavior of rename operation.</param>
    /// <returns>A <see cref="RenameFileResult"/> object containing the outcome.</returns>
    RenameFileResult RenameFile(RenameFileRequest renameFileRequest, RenameFileSettings settings);
}